...
resolver_service_file="/etc/systemd/system/rovyn-resolver.service"
resolved_dropin_dir="/etc/systemd/resolved.conf.d"
resolved_dropin_file="${resolved_dropin_dir}/rovyn.conf"
...
for required in \
  ...
  "${base_dir}/rovyn-resolver.service" \
  "${base_dir}/rovyn-resolved.conf" \
...
if [ -f "${resolver_service_file}" ]; then
  cp -a "${resolver_service_file}" "${backup_dir}/rovyn-resolver.service"
else
  : >"${backup_dir}/resolver-service.was-absent"
fi
if [ -f "${resolved_dropin_file}" ]; then
  cp -a "${resolved_dropin_file}" "${backup_dir}/rovyn-resolved.conf"
else
  : >"${backup_dir}/resolved-dropin.was-absent"
fi
...
rollback() {
  ...
  if [ -f "${backup_dir}/resolver-service.was-absent" ]; then
    systemctl disable --now rovyn-resolver.service >/dev/null 2>&1 || true
    rm -f "${resolver_service_file}"
  else
    cp -a "${backup_dir}/rovyn-resolver.service" "${resolver_service_file}"
  fi
  if [ -f "${backup_dir}/resolved-dropin.was-absent" ]; then
    rm -f "${resolved_dropin_file}"
  else
    install -d -m 0755 "${resolved_dropin_dir}"
    cp -a "${backup_dir}/rovyn-resolved.conf" "${resolved_dropin_file}"
  fi
...
systemd-analyze verify "${base_dir}/rovyn-resolver.service"

install -m 0644 "${base_dir}/99-rovyn-vpn.conf" "${sysctl_file}"
install -m 0644 "${base_dir}/rovyn-network-tuning.service" "${service_file}"
install -m 0644 "${base_dir}/rovyn-resolver.service" "${resolver_service_file}"
install -d -m 0755 "${resolved_dropin_dir}"
install -m 0644 "${base_dir}/rovyn-resolved.conf" "${resolved_dropin_file}"
...
systemctl enable --now rovyn-resolver.service
...
if ! curl --silent --show-error --fail \
  --resolve node.yanlo.biz:9443:127.0.0.1 \
  https://node.yanlo.biz:9443/health >/dev/null
then
